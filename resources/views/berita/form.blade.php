...
<style>
    ...
    .editor-wrapper .ql-editor { min-height:420px; line-height:1.7; color:var(--text-primary); }
...
<div class="form-wrap">
    ...
    <div class="page-header">
        <h2>
            <i class="fas fa-newspaper" style="color:var(--maroon-main);margin-right:8px;"></i>
            {{ $berita ? 'Edit Berita' : 'Tambah Berita' }}
        </h2>
        <p>{{ $berita ? 'Perbarui informasi berita.' : 'Buat berita baru untuk ditampilkan.' }}</p>
    </div>

    <form action="{{ $berita ? route('berita.update', $berita) : route('berita.store') }}"
          method="POST" enctype="multipart/form-data" id="beritaForm">
        @csrf
        @if($berita) @method('PUT') @endif

        {{-- Hidden textarea untuk nilai konten --}}
        <textarea name="konten" id="konten-hidden" style="display:none;">{{ old('konten', $berita?->konten) }}</textarea>

        <div class="card">
            <div class="card-header">
                <div class="card-header-icon"><i class="fas fa-pen-to-square"></i></div>
                <div class="card-header-title">Informasi Berita</div>
            </div>
            ...
            <div class="form-group">
                <label>Konten Berita <span class="req">*</span></label>
            ...
            <div class="form-group">
                ...
                <div class="radio-group">
                    <label class="radio-label">
                        ...
                        <i class="fas fa-pen" style="color:#6b7280"></i> Draft
                    </label>
                    <label class="radio-label">
                        <input type="radio" name="status" value="published"
                               {{ old('status', $berita?->status) === 'published' ? 'checked' : '' }}>
                        <i class="fas fa-globe" style="color:#10b981"></i> Published
                    </label>
                </div>
                <div class="form-hint">Draft tidak ditampilkan ke publik. Published akan langsung tampil.</div>
        ...
        <div class="card">
            <div class="card-header">
                ...
                <div class="card-header-title">Gambar Thumbnail (Opsional)</div>
            </div>
            ...
            <div class="form-group">
                ...
                <div class="form-hint">Gambar utama yang ditampilkan di daftar berita. Format JPG/PNG/WebP, maks. 2 MB.</div>
                @error('gambar')<div class="form-error">{{ $message }}</div>@enderror
                <div class="thumb-preview">
                    @if($berita?->gambar)
                        <img src="{{ Storage::url($berita->gambar) }}" id="preview-thumb" alt="Thumbnail">
                    @else
...
// Load konten yang sudah ada (edit mode / old input)
