...
<head>
    ...
    {{-- SEO --}}
    <title>{{ $berita->judul }} – KKA UM Kendari</title>
    ...
    <meta property="og:image"       content="{{ $berita->gambar ? asset(Storage::url($berita->gambar)) : asset('img/logo2.png') }}">
    ...
    <meta name="twitter:image"       content="{{ $berita->gambar ? asset(Storage::url($berita->gambar)) : asset('img/logo2.png') }}">
    ...
    <style>
        *  { margin:0; padding:0; box-sizing:border-box; }
        ...
        /* Article header */
        .article-label { display:inline-block; font-size:11px; font-weight:700; letter-spacing:.07em; text-transform:uppercase; color:var(--maroon-main); background:rgba(165,42,42,.08); border:1px solid rgba(165,42,42,.15); padding:3px 12px; border-radius:20px; margin-bottom:14px; }
        ...
        /* Hero image */
        .article-hero { width:100%; max-height:420px; object-fit:cover; border-radius:14px; margin-bottom:32px; box-shadow:0 4px 24px rgba(0,0,0,.1); }

        /* Content */
        .article-content { font-size:1rem; line-height:1.85; color:var(--text-primary); }
        .article-content h1,.article-content h2,.article-content h3 { margin:28px 0 12px; font-weight:700; line-height:1.3; color:var(--text-primary); }
...
<div class="page-wrap">
    ...
    <nav class="breadcrumb">
        ...
        <span>Berita</span>
        <i class="fas fa-chevron-right"></i>
        <span style="color:var(--text-primary);">{{ Str::limit($berita->judul, 50) }}</span>
    </nav>

    {{-- Article header --}}
    <div class="article-label"><i class="fas fa-newspaper"></i> Berita</div>
    <h1 class="article-title">{{ $berita->judul }}</h1>
    <div class="article-meta">
        <span><i class="fas fa-calendar-alt"></i>
            {{ ($berita->published_at ?? $berita->created_at)->translatedFormat('d F Y') }}
        </span>
        @if($berita->user)
    ...
    {{-- Hero image --}}
    @if($berita->gambar)
    <img src="{{ Storage::url($berita->gambar) }}" alt="{{ $berita->judul }}" class="article-hero">
    @endif
