...
<div class="dashboard-content">
    ...
    <div class="page-header">
        <div class="page-header-left">
            <h2>
                <i class="fas fa-building" style="color: var(--maroon-main); margin-right: 8px;"></i>
                Manajemen Fakultas
            </h2>
            <p>Data fakultas yang terdaftar di universitas</p>
        </div>

        @if(auth()->user()->hasAccess('tambah.fakultas'))
    ...
    {{-- Notifikasi jika user hanya bisa lihat --}}
    @if(!auth()->user()->hasAccess('tambah.fakultas') && !auth()->user()->hasAccess('edit.fakultas') && !auth()->user()->hasAccess('hapus.fakultas'))
    ...
    <div class="table-toolbar">
        <div class="search-box">
            ...
            <input type="text" id="searchInput" placeholder="Cari kode atau nama fakultas..." onkeyup="filterTable()">
        </div>
    ...
    <div class="table-container">
        ...
        <table>
            <thead>
                <tr>
                    ...
                    <th>Nama Fakultas</th>
                    <th>Keterangan</th>
                    @if(auth()->user()->hasAccess('edit.fakultas') || auth()->user()->hasAccess('hapus.fakultas'))
                    <th style="width: 130px;">Aksi</th>
                    @endif
                </tr>
            </thead>
            <tbody id="fakultasTableBody">
                @foreach($fakultas as $index => $item)
                <tr>
                    <td>{{ $index + 1 }}</td>
                    <td><span class="kode-badge">{{ $item->kode }}</span></td>
                    <td style="font-weight: 600;">{{ $item->nama }}</td>
                    <td style="color: var(--text-secondary); font-size: 13px;">{{ $item->keterangan ?? '-' }}</td>
                    @if(auth()->user()->hasAccess('edit.fakultas') || auth()->user()->hasAccess('hapus.fakultas'))
                    <td>
                        <div class="action-btns">
                            @if(auth()->user()->hasAccess('edit.fakultas'))
                            <button class="btn btn-warning btn-sm" title="Edit"
                                onclick="openEditModal({{ $item->id }}, '{{ addslashes($item->kode) }}', '{{ addslashes($item->nama) }}', '{{ addslashes($item->keterangan) }}')">
                                <i class="fas fa-edit"></i>
                            </button>
                            @endif

                            @if(auth()->user()->hasAccess('hapus.fakultas'))
        ...
        <div class="empty-state">
            ...
            <h3>Belum ada data fakultas</h3>
            @if(auth()->user()->hasAccess('tambah.fakultas'))
            <p>Klik tombol "Tambah Fakultas" untuk menambahkan data baru.</p>
            @else
...
{{-- MODAL TAMBAH (hanya render jika punya akses) --}}
@if(auth()->user()->hasAccess('tambah.fakultas'))
<div class="modal-overlay" id="modal-tambah">
    <div class="modal">
        <div class="modal-header">
            <h3><i class="fas fa-plus-circle" style="color: var(--maroon-main); margin-right: 8px;"></i>Tambah Fakultas</h3>
        ...
        <form action="{{ route('fakultas.store') }}" method="POST">
            ...
            <div class="modal-body">
                <div class="form-group">
                    ...
                    <input type="text" name="kode" placeholder="contoh: FKIP, FT, FH" required style="text-transform: uppercase;">
                    <div class="form-hint">Kode unik singkatan nama fakultas</div>
                </div>
                <div class="form-group">
                    <label>Nama Fakultas</label>
                    <input type="text" name="nama" placeholder="contoh: Fakultas Keguruan dan Ilmu Pendidikan" required>
                </div>
                <div class="form-group">
...
{{-- MODAL EDIT (hanya render jika punya akses) --}}
@if(auth()->user()->hasAccess('edit.fakultas'))
<div class="modal-overlay" id="modal-edit">
    <div class="modal">
        <div class="modal-header">
            <h3><i class="fas fa-edit" style="color: #f59e0b; margin-right: 8px;"></i>Edit Fakultas</h3>
...
{{-- MODAL HAPUS (hanya render jika punya akses) --}}
@if(auth()->user()->hasAccess('hapus.fakultas'))
...
<script>
    ...
    function openEditModal(id, kode, nama, keterangan) {
        ...
        document.getElementById('form-edit').action      = '/fakultas/' + id;
        openModal('modal-edit');
    }

    function openDeleteModal(id, nama) {
        document.getElementById('delete-nama').textContent = nama;
        document.getElementById('form-hapus').action       = '/fakultas/' + id;
        openModal('modal-hapus');
    }

    function filterTable() {
        const q = document.getElementById('searchInput').value.toLowerCase();
        document.querySelectorAll('#fakultasTableBody tr').forEach(row => {
