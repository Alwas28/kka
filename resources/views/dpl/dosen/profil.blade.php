...
<style>
    ...
    .breadcrumb i.sep { font-size: 9px; color: var(--text-secondary); }
    ...
    .tab-btn:not(.active) .tab-count {
        background: var(--gray-border); color: var(--text-secondary);
    }
    ...
    /* Data grid */
    .data-grid { display: grid; grid-template-columns: repeat(auto-fill, minmax(200px, 1fr)); gap: 0; }
    .data-row {
    ...
    .logbook-date {
        ...
        background: rgba(165,42,42,.07); border-radius: 6px; padding: 3px 8px;
        white-space: nowrap;
    }
    .logbook-text { font-size: 12px; color: var(--text-primary); line-height: 1.5; }
    .logbook-lokasi { font-size: 11px; color: var(--text-secondary); margin-top: 3px; }
    ...
    .grade-display {
        ...
        padding: 16px 20px; border-radius: 12px; margin-bottom: 18px;
    ...
    .grade-ab { color: #2563eb; }
    .grade-b  { color: #0284c7; }
    .grade-bc { color: #d97706; }
...
<div class="dashboard-content">
    ...
    <div class="breadcrumb">
        ...
        <a href="{{ route('dosen-pembimbing.detail', $kegiatan->id) }}">{{ $kegiatan->nama }}</a>
        <i class="fas fa-chevron-right sep"></i>
    ...
    <div class="tab-panel active" id="tab-data-diri">
        ...
        @if($pendaftaran->penyakit_diderita || $pendaftaran->catatan_kesehatan || $pendaftaran->sedang_hamil)
        <div class="info-card">
            <div class="info-card-header">
                <i class="fas fa-heartbeat"></i> Informasi Kesehatan
            </div>
            <div class="info-card-body" style="padding:0;">
                <div class="data-grid">
    ...
    <div class="tab-panel" id="tab-dokumen">
        ...
        <div class="info-card">
            <div class="info-card-header">
                <i class="fas fa-file-upload"></i> Dokumen yang Diunggah
            </div>
                ...
                <table class="logbook-tbl">
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td>
                                <div class="logbook-text">{{ $lb->kegiatan_dilakukan }}</div>
                            </td>
                            <td>
                                @if($lb->lokasi)
                                <div class="logbook-lokasi">
        ...
        <div class="info-card">
            <div class="info-card-body">
                <div class="empty-state">
                    ...
                    <p>Belum ada entri logbook dari mahasiswa ini.</p>
                </div>
            </div>
        </div>
        @endif
    ...
    <div class="tab-panel" id="tab-laporan">
        ...
        <div class="info-card">
            <div class="info-card-header">
                <i class="fas fa-file-alt"></i> Laporan Individu
            </div>
            <div class="info-card-body">
                ...
                <div class="laporan-item">
                    ...
                    <a href="{{ asset('storage/' . $lap->file_path) }}" target="_blank" class="btn-dl" style="flex-shrink:0;">
                        <i class="fas fa-download"></i> Unduh
                    </a>
                </div>
                @endforeach
        ...
        {{-- Laporan Akhir (hanya jika koordinator) --}}
        @if($surveyLokasi->is_koordinator)
        <div class="info-card">
            <div class="info-card-header">
            ...
            <div class="info-card-body">
                ...
                @php
                    ...
                    $icon = $ext === 'pdf' ? 'fa-file-pdf' : (in_array($ext, ['doc','docx']) ? 'fa-file-word' : 'fa-file-alt');
                    $sizeKb = $laporanAkhir->file_size ? round($laporanAkhir->file_size / 1024) . ' KB' : '';
                @endphp
                <div class="laporan-item">
                    <div class="laporan-icon"><i class="fas {{ $icon }}"></i></div>
                    <div class="laporan-item-info">
                    ...
                    <a href="{{ asset('storage/' . $laporanAkhir->file_path) }}" target="_blank" class="btn-dl" style="flex-shrink:0;">
                        <i class="fas fa-download"></i> Unduh
                    </a>
                </div>
                @else
    ...
    <div class="tab-panel" id="tab-nilai">
        ...
        <div class="komponen-list">
            ...
            <span class="komponen-chip">{{ $kp->nama }} — <strong>{{ $kp->persentase }}%</strong></span>
            @endforeach
        </div>
        @endif
        ...
        <div class="nilai-cards">
            <div class="nilai-card">
                <div class="nilai-card-label">
                    {{ $komponenPenilaian->get(0)->nama ?? 'Logbook' }}
                </div>
                <div class="nilai-card-val {{ $nilai && $nilai->nilai_logbook !== null ? '' : 'na' }}">
                    {{ $nilai && $nilai->nilai_logbook !== null ? number_format($nilai->nilai_logbook, 1) : '—' }}
                </div>
                <div class="nilai-card-sub">
                    @if($komponenPenilaian->get(0)) {{ $komponenPenilaian->get(0)->persentase }}% @endif
                </div>
            </div>
            <div class="nilai-card">
                <div class="nilai-card-label">
                    {{ $komponenPenilaian->get(1)->nama ?? 'Laporan Individu' }}
                </div>
                <div class="nilai-card-val {{ $nilai && $nilai->nilai_laporan_individu !== null ? '' : 'na' }}">
                    {{ $nilai && $nilai->nilai_laporan_individu !== null ? number_format($nilai->nilai_laporan_individu, 1) : '—' }}
                </div>
                <div class="nilai-card-sub">
                    @if($komponenPenilaian->get(1)) {{ $komponenPenilaian->get(1)->persentase }}% @endif
                </div>
            </div>
            <div class="nilai-card">
                <div class="nilai-card-label">
                    {{ $komponenPenilaian->get(2)->nama ?? 'Laporan Akhir' }}
                </div>
                <div class="nilai-card-val {{ $nilai && $nilai->nilai_laporan_akhir !== null ? '' : 'na' }}">
                    {{ $nilai && $nilai->nilai_laporan_akhir !== null ? number_format($nilai->nilai_laporan_akhir, 1) : '—' }}
                </div>
                <div class="nilai-card-sub">
                    @if($komponenPenilaian->get(2)) {{ $komponenPenilaian->get(2)->persentase }}% @endif
                </div>
            </div>
            <div class="nilai-card akhir">
                <div class="nilai-card-label">Nilai Akhir</div>
                <div class="nilai-card-val {{ $naVal !== null ? '' : 'na' }}">
        ...
        <div class="grade-display">
            ...
            <div class="grade-info">
                ...
                @php
                    $gradeObj = $gradeTable->first(fn($g) => $g->grade === $gradeStr);
                @endphp
                @if($gradeObj)
        ...
        <div class="info-card">
            <div class="info-card-header">
                <i class="fas fa-comment-alt"></i> Catatan DPL
            </div>
            <div class="info-card-body">
        ...
        <div class="info-card">
            <div class="info-card-body">
                <div class="empty-state">
                    ...
                    <p>Nilai untuk mahasiswa ini belum diinputkan.</p>
                </div>
            </div>
        </div>
        @endif
