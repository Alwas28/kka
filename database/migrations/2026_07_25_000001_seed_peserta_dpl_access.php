...
return new class extends Migration
{
    public function up(): void
    {
        DB::table('accesses')->insertOrIgnore([
            ['nama' => 'lihat.peserta', 'keterangan' => 'Lihat Daftar Peserta KKA', 'created_at' => now(), 'updated_at' => now()],
            ['nama' => 'lihat.dpl',     'keterangan' => 'Lihat Daftar DPL yang Ditugaskan', 'created_at' => now(), 'updated_at' => now()],
        ]);

        $adminRole = DB::table('roles')->where('nama', 'Administrator')->value('id');

        foreach (['lihat.peserta', 'lihat.dpl'] as $nama) {
    ...
    public function down(): void
    {
        $ids = DB::table('accesses')
            ->whereIn('nama', ['lihat.peserta', 'lihat.dpl'])
            ->pluck('id');
